...
import transformers
import evaluate
from evaluation import evaluation_baseline_lm
...
def load_model(args, load=None):
    ...
    if load:
        if args.is_2lr:
            lm_base_model.load_state_dict(torch.load(load.split('.')[0] + '_lm.pt'))
            rgcn_model.load_state_dict(torch.load(load.split('.')[1] + '_rgcn.pt'))
...
def load_configs(args, model, train_loader):
    ...
    return (optimizer_lm), (scheduler_lm)
#################################################################
################## TRAIN ########################################
#################################################################
...
def train_epoch(args, epoch, model, train_loader, optimizer, scheduler, device):
    ...
    total_loss = 0
    optimizer_lm = optimizer
    scheduler_lm = scheduler
...
def train(args, model, train_loader, val_loader, val_dataset, patience, delta, device):
    ...
    # lm_model.load_state_dict(torch.load(args.model_path.split('.')[0] + '_lm.pt'))
...
def arg_parse():
    ...
    parser.add_argument('--model_path', type=str, default='baseline_model_q8.pt')
    parser.add_argument('--samples_file', type=str, default='baseline_model_q8.txt')
...
if __name__ == '__main__':
    args = arg_parse()
    wandb.init(project="pssp_main_q8", name="baseline_model_q8")
    config = {
        "epochs": args.epochs,
    ...
    print("Training")
    train(args, model, train_loader, val_loader, val_dataset, args.patience, args.delta, device)
